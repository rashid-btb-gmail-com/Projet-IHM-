...
if (isset($_GET['t'],$_GET['id'],$_GET['id_cl']) and !empty($_GET['t']) and !empty($_GET['id']) and !empty($_GET['id_cl'])) {
	...
	$id_cl=(int) $_GET['id_cl'];
	 ...
	 $req = $db->prepare('SELECT id FROM biens_location where id=?');
	 ...
	 if ($req->rowCount()== 1) {
	 	if($gett==1){
	 		$fav=$db->prepare('SELECT id FROM favoris_loc where id_biens=? and id_cl=?');
	 		...
	 		$ins=$db->prepare('INSERT INTO favoris_loc(id_biens,id_cl) VALUES (?,?)');
